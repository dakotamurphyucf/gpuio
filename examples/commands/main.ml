...
let component ~phase ~observed window graph =
  ...
  View.command_scope
    ~commands
    ~style
    [ View.text "Command-K on macOS, Control-K on Linux: run the focused scope's command."
...
let () =
  ...
  then (
    assert !completed;
    print_endline
      "GPUIO_COMMANDS_PUBLIC_OK: public nested registries, native render \
       acknowledgements, disable/re-enable, scope removal and clean shutdown")
;;
