...
let component ~phase_var ~events ~ready ~self_test _window graph =
  ...
  let config =
    Animation.Config.create
      ...
      ~duration:
        (if self_test && phase >= 2
         then Time_ns.Span.of_sec 60.
         else Time_ns.Span.of_ms 220.)
...
let () =
    ...
    (fun env app ->
       ...
       let window =
         App.open_window
           ...
           ~width:640.
           ~height:260.
           (component ~phase_var ~events ~ready ~self_test)
         |> Or_error.ok_exn
       in
       if self_test
       then
  ...
  then (
    assert !completed;
    Eio.traceln
      "GPUIO_ANIMATION_PUBLIC_OK: Bonsai targets, native endpoints, live reduced motion, \
       theme update and clean shutdown")
;;
