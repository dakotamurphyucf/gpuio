...
module type S = sig
  module Private_computation : sig
    type 'a t
  end

  module Private_value : sig
    type 'a t
  end
  ...
  *)
  ...
  (** Given a value containing the current state (like from a [Bonsai.state] or
      [Bonsai.state_machine]), [narrow] gives you access to a subset of the state and a setter
      for the subset of that type.
  ...
  *)
  ...
  module Let_syntax : sig
    ...
    module Let_syntax : sig
      ...
      val map2 : 'a Value.t -> 'b Value.t -> f:('a -> 'b -> 'c) -> 'c Value.t
      val return : 'a Value.t -> 'a Computation.t
  ...
  module Map :
    Map0_intf.Output with module Value := Value and module Computation := Computation
end
