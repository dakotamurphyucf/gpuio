...
Keep one in-flight tree transaction per window initially. Reconcile the newest desired state against the acknowledged state; do not queue obsolete full trees. Track transaction acceptance separately from rendering completion and logical no-change cycles. The v0.17 lifecycle fixes remain part of this contract. Use one shared periodic Eio clock plus immediate action, work-completion and requested-frame wakeups; clocks/after-display subscriptions still need scheduling even when pixels do not change. Unchanged cycles do not force native rendering.
