...
let px value =
  if valid value
  then Ok (Wire.Length.Px value)
  else Or_error.error_string "invalid logical pixel length"
;;
...
let percent value =
  if valid value
  then Ok (Wire.Length.Percent value)
  else Or_error.error_string "invalid percentage length"
;;
