...
val button : ?key:Key.t -> ?style:Style.t -> on_click:(unit -> 'action) -> string -> 'action t
val row : ?key:Key.t -> ?style:Style.t -> 'action t list -> 'action t
val column : ?key:Key.t -> ?style:Style.t -> 'action t list -> 'action t
val grid : ?key:Key.t -> ?style:Style.t -> columns:int -> 'action t list -> 'action t Or_error.t

module Expert : sig
  module Kind : sig
    type t = Container | Text | Button [@@deriving equal, sexp_of]
  end
