...
module Property = struct
  ...
  let expand = function
    ...
    | Border_width v ->
      [ Border_top_width v
      ; Border_right_width v
      ; Border_bottom_width v
      ; Border_left_width v
      ]
    | Radius v ->
      [ Top_left_radius v
      ; Top_right_radius v
      ; Bottom_left_radius v
      ; Bottom_right_radius v
      ]
  ...
  let validate property =
    ...
    let length v ~auto ~negative =
      match Length.Expert.to_wire v with
      | Auto -> auto
      | Px n | Percent n -> negative || Float.(n >= 0.)
    in
    let valid =
      match property with
      | Display _ -> true
    ...
    if valid
    then Ok ()
    else Or_error.error_s [%message "invalid style property" (property : t)]
  ;;
end
...
let with_state t state properties =
  if List.length properties > 128
  then Or_error.error_string "too many style declarations"
  else (
    let properties = List.concat_map properties ~f:Property.expand in
    let%map.Or_error properties =
      List.fold_result properties ~init:Property.Name.Map.empty ~f:(fun result property ->
        let%bind.Or_error () = Property.validate property in
        let%map.Or_error () =
          ...
          | _, _ -> Ok ()
        in
        Map.set result ~key:(Property.name property) ~data:(Some property))
    in
...
let merge styles =
  List.fold styles ~init:empty ~f:(fun result next ->
...
module Expert = struct
  ...
  let fill theme background =
    match Background.Expert.describe background with
    | Solid c ->
  ...
  let field theme = function
    ...
    | Property.Justify_content v ->
      Ok (Wire.Field.Justify_content (Distribution.to_int64 v))
    ...
    | Property.Grid_column_minimum v ->
      Ok (Wire.Field.Grid_column_minimum (Grid_minimum.to_int64 v))
    | Property.Grid_row_minimum v ->
    ...
    | Property.Padding_bottom v ->
      Ok (Wire.Field.Padding_bottom (Length.Expert.to_wire v))
    ...
    | Property.Background v ->
      let%map.Or_error v = fill theme v in
      Wire.Field.Background v
    | Property.Foreground v ->
      let%map.Or_error v = color theme v in
      Wire.Field.Foreground v
    ...
    | Property.Text_decoration v ->
      Ok (Wire.Field.Text_decoration (Text_decoration.to_int64 v))
    ...
    | Property.Selection_color v ->
      let%map.Or_error v = color theme v in
      Wire.Field.Selection_color v
    | Property.Accessible_name v -> Ok (Wire.Field.Accessible_name v)
    | Property.Padding _ | Margin _ | Gap _ | Border_width _ | Radius _ | Overflow _ ->
      assert false
  ;;

  let to_wire t ~theme =
    Map.to_alist t
    |> List.map ~f:(fun (state, properties) ->
      let%map.Or_error fields =
        Map.data properties
        |> List.filter_opt
        |> List.map ~f:(field theme)
        |> Or_error.all
      in
      if state = 0
      then Wire.Style.Fields fields
      else Wire.Style.State (Int64.of_int state, fields))
    |> Or_error.all
  ;;
end
